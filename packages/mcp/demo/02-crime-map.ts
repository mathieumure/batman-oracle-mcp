...
mcpServer.registerTool(
  ...
  async ({ city, connectChronologically = false, fingerprint, molecule, suspect }) => {

    const data = await GCPDClient.getCrimes({ city, fingerprint, molecule, suspect });

    const payload = {
        crimesInfo: {
          total: data.crimes.length,
          molecules: Array.from(new Set(data.crimes.flatMap((it) => it.forensics.molecules))),
          fingerprints: Array.from(new Set(data.crimes.flatMap((it) => it.forensics.fingerprints))),
        },
    };
